...
class OssAdapter implements FilesystemAdapter
{
    ...
    {
        try {
            $this->client->putObject($this->bucket, $this->prefixer->prefixPath($path), $contents);
        } catch (OssException $exception) {
    ...
    {
        try {
            $this->client->uploadStream($this->bucket, $this->prefixer->prefixPath($path), $contents);
        } catch (OssException $exception) {
            throw UnableToWriteFile::atLocation($path, $exception->getErrorCode(), $exception);
        }
    }
    ...
    public function readStream(string $path)
    {
        ...
        try {
            $this->client->getObject($this->bucket, $this->prefixer->prefixPath($path), [OssClient::OSS_FILE_DOWNLOAD => $stream]);
        } catch (OssException $exception) {
            fclose($stream);
            throw UnableToReadFile::fromLocation($path, $exception->getErrorCode(), $exception);
        }
    ...
    public function deleteDirectory(string $path): void
    {
        try {
            ...
            $this->directoryExists($path) && $this->client->deleteObject($this->bucket, $this->prefixer->prefixDirectoryPath($path));
        } catch (OssException $exception) {
    ...
    {
        try {
            $this->client->putObject($this->bucket, $this->prefixer->prefixDirectoryPath($path), '');
        } catch (OssException $exception) {
